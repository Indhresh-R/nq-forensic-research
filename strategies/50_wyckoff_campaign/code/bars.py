...
def to_ticks(price: float | np.ndarray) -> np.ndarray | int:
    """Nearest-tick integer grid. Scalar in → Python int; array in → int64 array."""
...
def causal_atr20(bars: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    # invalidate prev_close across segment breaks
    ...
    # rolling mean within segment via groupby
    s = pd.Series(tr)
    atr_at = s.groupby(seg, sort=False).transform(lambda x: x.rolling(ATR_N, min_periods=ATR_N).mean()).to_numpy(
        np.float64
    )
