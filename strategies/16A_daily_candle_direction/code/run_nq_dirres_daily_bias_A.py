"""
Hypothesis 16A — Prior Globex daily candle direction as resolver under Strategy-12 HIGH

Resolver-only. Bias A only. No stops/targets/RR. No Bias B/C/D.

Question:
  Does sign(prior session_date close - open) resolve NQ forward returns
  better inside HIGH than unconditionally (same session/T)?

Daily candle = full Globex session_date day (rolls 18:00 ET). Prior day completed
before any T on current session_date.
"""
...
# Pre-specified — do not select after seeing results
        ...
        rows.append(
            {
                ...
                "day_close": c,
                "day_range": h - l,
                "bias_A": (1 if c > o else (-1 if c < o else 0)),
            }
        )
    daily = pd.DataFrame(rows).sort_values("session_date").reset_index(drop=True)
    daily["prior_bias_A"] = daily["bias_A"].shift(1)
...
def main() -> None:
    print("=== Hypothesis 16A: Prior Globex daily Bias A under Strategy-12 HIGH ===", flush=True)
    print(f"H pre-specified={HOLD_HS}; primary={PRIMARY_HS}; Bias A only", flush=True)

    df = load_nq()
    daily = build_globex_daily(df)
    daily_path = art("nq_dirres_dailyA_globex_days.parquet")
    ...
    for (sd, name), bars in sess_bars.items():
        ...
        if dr is None:
            continue
        bias = dr["prior_bias_A"]
        if not np.isfinite(bias) or int(bias) == 0:
            continue
        side = int(bias)
        psr = float(fr["psr"])
        ...
        for off in SESSION_DECISION_OFFSETS[name]:
            ...
            # Need max horizon room; store each H that fits
            entry = float(after.iloc[0]["open"]) if len(after) else np.nan
            if not np.isfinite(entry) or len(after) < min(HOLD_HS):
                continue
            ...
            row: dict[str, Any] = {
                ...
                "regime": reg,
                "side_bias": side,
                "prior_open": float(dr["prior_day_open"]),
                "prior_close": float(dr["prior_day_close"]),
                "rng_psr": float(st["rng_psr"]),
            }
            any_h = False
            for H in HOLD_HS:
                if len(closes) < H:
                    row[f"pnl_bias_{H}"] = np.nan
                    row[f"pnl_long_{H}"] = np.nan
                    continue
                px = float(closes[H - 1])
                row[f"pnl_bias_{H}"] = side * (px - entry)
                row[f"pnl_long_{H}"] = 1.0 * (px - entry)
                any_h = True
            if any_h:
                rows.append(row)
        n_done += 1
    ...
    panel_path = art("nq_dirres_dailyA_panel.parquet")
    ...
    res_df.to_csv(art("nq_dirres_dailyA_results.csv"), index=False)
    ...
    for sess in SESSION_PRIORITY:
        for off in SESSION_DECISION_OFFSETS[sess]:
            for H in HOLD_HS:
                hb = get(sess, off, H, "HIGH_bias", "IS")
                ab = get(sess, off, H, "ALL_bias", "IS")
                hl = get(sess, off, H, "HIGH_long", "IS")
                if not hb or not ab or hb.get("n", 0) < 80 or ab.get("n", 0) < 80:
                    continue
                hv = get(sess, off, H, "HIGH_bias", "Validation")
                av = get(sess, off, H, "ALL_bias", "Validation")
                ho = get(sess, off, H, "HIGH_bias", "OOS")
                ao = get(sess, off, H, "ALL_bias", "OOS")
                ...
                # Prefer HIGH+bias over coin-flip and over HIGH long
                high_ok = hb["win"] >= 0.52 or hb["E"] > 0
                beats_long = True
                if hl and hl.get("n", 0) >= 80:
                    beats_long = (hb["win"] - hl["win"] >= -0.005) or (hb["E"] - hl["E"] >= 0)
                y25h = get(sess, off, H, "HIGH_bias", "Y2025")
                y25a = get(sess, off, H, "ALL_bias", "Y2025")
                y26h = get(sess, off, H, "HIGH_bias", "Y2026")
                y26a = get(sess, off, H, "ALL_bias", "Y2026")
                year_ok = False
                if (
                    y25h
                    and y25a
                ...
                strong = strong_is and vo_ok and high_ok and year_ok and beats_long
                if soft or strong:
                    candidates.append(
                        {
                            "session": sess,
                            ...
                            "HIGH_bias_IS_n": hb["n"],
                            "HIGH_bias_IS_win": hb["win"],
                            "ALL_bias_IS_win": ab["win"],
                            "HIGH_long_IS_win": hl.get("win", np.nan) if hl else np.nan,
                            "lift_win_IS": lw_is,
                            "lift_E_IS": le_is,
                            "lift_win_Val": lw_v,
                            "lift_win_OOS": lw_o,
                            "HIGH_bias_OOS_win": ho["win"],
                            "ALL_bias_OOS_win": ao["win"],
                            "HIGH_long_OOS_win": get(sess, off, H, "HIGH_long", "OOS").get(
                                "win", np.nan
                            ),
                            "year_ok": year_ok,
                        }
    ...
    cand_df.to_csv(art("nq_dirres_dailyA_candidates.csv"), index=False)
    ...
    # Compact all-H OOS lift heatmap style table for London (priority session)
    lines.append("## London lift vs ALL by H (best IS clock per H)")
    lines.append("")
    lines.append("| H | T+ | HIGH+bias IS | ALL+bias IS | Lift IS | Lift Val | Lift OOS |")
    lines.append("|---|----|--------------|-------------|---------|----------|----------|")
    for H in HOLD_HS:
        best = None
        for off in SESSION_DECISION_OFFSETS["LONDON"]:
            hb = get("LONDON", off, H, "HIGH_bias", "IS")
            ab = get("LONDON", off, H, "ALL_bias", "IS")
            if not hb or not ab or hb.get("n", 0) < 40:
                continue
            lift = hb["win"] - ab["win"]
            if best is None or lift > best["lift"]:
                hv = get("LONDON", off, H, "HIGH_bias", "Validation")
                av = get("LONDON", off, H, "ALL_bias", "Validation")
                ho = get("LONDON", off, H, "HIGH_bias", "OOS")
                ao = get("LONDON", off, H, "ALL_bias", "OOS")
                best = {
                    "off": off,
                    "lift": lift,
                    "hb": hb,
    ...
    if n_strong == 0 and n_soft == 0:
        verdict = "C"
        call = (
            "Kill 16A. Prior Globex daily Bias A does not resolve direction "
            "conditionally inside Strategy-12 HIGH vs unconditional baseline."
        )
    elif multi_strong:
        verdict = "B_review"
        call = (
            f"Multi-clock strong lift keys={len(multi_strong)}. Interesting scientific "
            "finding — still not a strategy; freeze before any econ/execution card."
        )
    elif n_soft:
        verdict = "B->kill"
        call = (
    ...
    lines.append("")
    lines.append("- This pass is **resolver-only** (no trade card).")
    lines.append("- If killed: next is **16B as a separate dossier**, not a parameter rescue.")
    lines.append("")

    art("nq_dirres_dailyA_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    art("nq_dirres_dailyA_report.json").write_text(
        json.dumps(
            {
                "verdict": verdict,
